...
class CharacterEditView extends PureComponent<Props> {
    ...
    return <div className={classes}>
      ...
      >
        <h3 className={'instructions'}>
          ...
          <button type={'button'}
            className={'small'}
            onClick={() => this.props.showModal('instructions', this.instructionsModal(), false)}>
      ...
      <div className={'selected-characters'}>
        <h4>
          ...
          <div className="character-list-actions">
            ...
            <button
              className={'small'}
              onClick={() => this.props.showModal('generate-character-list', this.generateCharacterListModal(), false)}
            >
              Auto-generate List
            </button>
          </div>
        ...
        <div className={'template-buttons'}>
          <div className={'row'}>
            ...
            <button className={'small'}
              ...
              onClick={() => this.props.showModal('save-template', this.saveTemplateModal(), false)}>
              Save
            </button>
            <button className={'small'}
              disabled={!this.userTemplates().length}
              onClick={() => this.props.showModal('export-template', this.exportTemplateModal(), false)}>
              Export
            </button>
                ...
                (templates) => {
                  try {
                    ...
                    const templatesDeserialized = templatesObject.map(t => ({
                      name: t.name,
                      selectedCharacters: t.selectedCharacters.map(({ id, target }: {id: CharacterNames, target: OptimizationPlan}) => ({
                        id: id,
            ...
            <button className={'small red'}
              disabled={!this.userTemplates().length}
              onClick={() => this.props.showModal('delete-template', this.deleteTemplateModal(), false)}>
              Delete
            </button>
          </div>
          <div className={'row'}>
            Apply:
            <button className={'small'}
              onClick={() => this.props.showModal('append-template', this.appendTemplateModal(), false)}>
              Append
             </button>
            <button className={'small'}
              onClick={() => this.props.showModal('replace-template', this.replaceTemplateModal(), false)}>
              Replace
             </button>
            <button className={'small'}
              onClick={() => this.props.showModal('template-targets', this.templateTargetsModal(), false)}>
              Apply targets only
            </button>
          </div>
        </div>
  ...
  sidebarActions() {
    return <div className={'sidebar-actions'} key={'sidebar-actions'}>
      ...
      <button
        ...
        onClick={() => {
          ...
          const duplicateCharacters: CharacterNames[] = keysWhere(
            collectByKey(this.props.selectedCharacters, ({ id }: {id: CharacterNames}) => id),
            (targets: SelectedCharacters) => targets.length > 1) as CharacterNames[];


            type IndexOfCharacters = {[id in CharacterNames]: number};
            const minCharacterIndices: IndexOfCharacters = this.props.selectedCharacters.reduce(
              ...
              this.props.showError([
                ...
                <ul>
                  {invalidTargets.map(id => <li>
                    {this.props.baseCharacters[id] ? this.props.baseCharacters[id].name : id}
                  </li>)}
                </ul>
              ]);
            } else if (duplicateCharacters.length > 0 || hasTargetStats) {
            this.props.showModal('notice', this.optimizeWithWarningsModal(duplicateCharacters, hasTargetStats), false);
          } else {
      ...
      <button
        type={'button'}
        className={'blue'}
        onClick={() => this.props.showModal('reset-modal', this.resetCharsModal(), false)}
      >
        Reset all targets
      </button>
    </div>
  ...
  saveTemplateModal() {
    const isNameUnique = (name: string) => !this.props.characterTemplates.includes(name);
  ...
  replaceTemplateModal() {
    ...
    return <div>
      ...
      {this.templateSelectElement((select: HTMLSelectElement) => templateSelection = select)}
      <div className={'actions'}>
        <button type={'button'} onClick={() => this.props.hideModal()}>Cancel</button>
    ...
    return <div>
      ...
      <div className={'actions'}>
        ...
        <button type={'button'} onClick={() => {
          templateSelection && this.props.applyTemplateTargets(templateSelection.value)
        }}>
          Apply Targets
        </button>
      </div>
  ...
  exportTemplateModal() {
    ...
    return <div>
      ...
      <div className={'actions'}>
        ...
        <button type={'button'}
          onClick={() =>
            (templateNameInput !== null) && this.props.exportTemplate(
        ...
        <button type={'button'}
          onClick={() => this.props.exportAllTemplates((templates: CharacterTemplates) => {
   ...
   * @returns {*}
   */
  optimizeWithWarningsModal(duplicates: CharacterNames[], hasTargetStats: boolean) {
    return <div>
      ...
      {duplicates.length > 0 && <div>
        ...
        <ul>
          {duplicates.map((characterID: CharacterNames) =>
            <li>{this.props.baseCharacters[characterID] ? this.props.baseCharacters[characterID].name : characterID}</li>
          )}
        </ul>
        <hr />
      </div>}
...
const mapStateToProps = (state: IAppState) => {
  ...
  return {
    ...
    availableCharacters: availableCharacters ? availableCharacters.filter(c => !characterFilter(c)) : [],
...
const mapDispatchToProps = (dispatch: ThunkDispatch) => ({
  showModal: (clazz: string, content: DOMContent, cancelable: boolean) => dispatch(showModal(clazz, content, cancelable)),
  hideModal: () => dispatch(hideModal()),
  showError: (error: DOMContent) => dispatch(showError(error)),
  changeCharacterFilter: (filter: string) => dispatch(changeCharacterFilter(filter)),
  ...
  updateLockUnselectedCharacters: (lock: boolean) => dispatch(updateLockUnselectedCharacters(lock)),
  ...
  updateModChangeThreshold: (threshold: number) => dispatch(updateModChangeThreshold(threshold)),
  updateForceCompleteModSets: (forceCompleteModSets: boolean) => dispatch(updateForceCompleteModSets(forceCompleteModSets)),
  generateCharacterList: (mode: UseCaseModes, behavior: boolean, allyCode: string, parameters: CharacterListGenerationParameters) => {
    dispatch(fetchCharacterList(mode, behavior, allyCode, parameters));
    dispatch(hideModal());
  },
  saveTemplate: (name: string) => dispatch(saveTemplate(name)),
  saveTemplates: (templates: CharacterTemplates) => dispatch(saveTemplates(templates)),
  ...
  exportTemplate: (templateName: string, callback: (template: CharacterTemplate) => void) => {
  ...
  deleteTemplate: (templateName: string) => dispatch(deleteTemplate(templateName))
});
