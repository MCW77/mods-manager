...
// state
import { type ObservableObject, beginBatch, endBatch, observable } from "@legendapp/state";
...
import { type PersistedProfiles, profilesManagement$ } from "#/modules/profilesManagement/state/profilesManagement";
import { templates$ } from "#/modules/templates/state/templates";
import { ui$ } from "#/modules/ui/state/ui";

// domain
import { type PlayerProfile, type PersistedPlayerProfile, getProfileFromPersisted }from "#/modules/profilesManagement/domain/PlayerProfile";
...
export const appState$: ObservableObject<AppState> = observable({
  reset: () => {
    beginBatch();
    dialog$.hide();
    hotutils$.reset();
  ...
  loadBackup: (serializedAppState: string) => {
    try {
      ...
      const profiles: PlayerProfile[] = Object.values(profilesManagement.profileByAllycode).map(
        (profile: PersistedPlayerProfile) => getProfileFromPersisted(profile),
      );
      for (const profile of profiles) {
        if (profilesManagement$.hasProfileWithAllycode(profile.allycode)) {
          profilesManagement$.profiles.profileByAllycode[profile.allycode].set(profile);
        }
      }
...
//      dispatch(Storage.thunks.saveLastRuns(stateObj.lastRuns));
      ...
      if ((profilesManagement.activeAllycode !== "") && (profilesManagement$.hasProfileWithAllycode(profilesManagement.activeAllycode))) {
        profilesManagement$.profiles.activeAllycode.set(profilesManagement.activeAllycode);
