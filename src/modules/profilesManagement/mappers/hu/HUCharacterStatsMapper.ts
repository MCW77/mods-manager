...
class HUCharacterStatsMapper {
  ...
  static fromHU(statsDTO: DTOs.HU.HUCharacterStatsDTO | undefined): DTOs.GIMO.CharacterStatsDTO {
    if (statsDTO === undefined) return {
      Health: 0,
      Protection: 0,
      Speed: 0,
      ['Potency %']: 0,
    ...
    return {
      ...
      ['Critical Avoidance %']: (statsDTO['Physical Critical Avoidance'] ?? 0) / 24,
      ['Critical Damage %']: (statsDTO['Critical Damage'] ?? 0) * 100,
      Health: statsDTO.health ?? 0,
      ['Physical Critical Chance %']: statsDTO['Physical Critical Chance'] ? statsDTO['Physical Critical Chance'] / 24 + 10 : 0,
      ...
      ['Special Critical Chance %']: statsDTO['Special Critical Chance'] ? statsDTO['Special Critical Chance'] / 24 + 10 : 0,
