...
import { type CharacterNames, characterSettings } from "#/constants/characterSettings";
...
const isCompilation = (compilation$: Observable<Compilation> | Observable<Compilation | undefined>): compilation$ is Observable<Compilation> => {
...
const compilations$ = observable({
  ...
  ensureSelectedCharactersExist: (compilationId: string) => {
    ...
    const compilation$ = compilations$.compilationByIdByAllycode[profile$.allycode.peek()][compilationId];
...
console.log("Waiting for compilations to load");
...
console.log("Waiting for DefaultCompilation to load");
await when(syncStatus2$.isPersistLoaded);
console.log("DefaultCompilation loaded");
/*
(async () => {
	await when(syncStatus2$.isPersistLoaded);
})();
