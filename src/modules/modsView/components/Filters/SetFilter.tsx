	...
	React.memo(() => {
		...
		return (
			<div className={"w-24 flex flex-col gap-2 items-center"}>
				...
				<div id={"set-filter1"} className="flex flex-row justify-center flex-wrap">
					{Object.keys(setConfig).map((set: keyof SetSettings) => {
						const inputName = `set-filter-${set}`;
						const value = setConfig[set] || 0;
						const setCSS = `w-[2.9em] h-[2.9em] p-[.2em] bg-origin-content bg-clip-content bg-[url('../../img/icon-buffs.png')] bg-[length:20em_5em] ${imageOffsets[set][value]} bg-no-repeat`;
            const stateCSS =
              value === 0
								? "opacity-50"
								: "opacity-100";
