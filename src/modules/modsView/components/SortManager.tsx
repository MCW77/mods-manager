...
const SortManager = observer(
    ...
    const statOptions = SecondaryStats.SecondaryStat.statNames.map(stat =>
      <SelectItem value={`Stat${stat}`} key={stat}>{t(`stats.${stat}`)}</SelectItem>
    );
    const statScoreOptions = SecondaryStats.SecondaryStat.statNames.map(stat =>
      <SelectItem value={`StatScore${stat}`} key={stat}>{t(`stats.${stat}`)}</SelectItem>
    );

    const modScoreOptions = ModScoresConsts.modScores.map(modScore =>
      <SelectItem value={`ModScore${modScore.name}`} key={modScore.name} title={modScore.description}>{modScore.displayName}</SelectItem>
    );
