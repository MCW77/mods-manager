...
const ModSchema = v.pipe(
  v.object({
    id: v.string(),
    inLoadout: v.boolean(),
    loadouts: v.nullable(v.array(v.object({
  ...
  v.transform((data) => {
    ...
    const speedStat = data.secondaryStat.find((stat) => stat.stat.unitStatId === 5);
    if (speedStat) {
...
type FetchedFullGIMOProfile = v.InferOutput<typeof FetchedFullGIMOProfileSchema>;
