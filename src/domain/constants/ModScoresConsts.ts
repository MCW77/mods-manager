...
modScores.push(new ModScore(
  ...
  (mod: Mod) => {
    ...
    return parseFloat((mod.secondaryStats as (PrimaryStats.PrimaryStat | SecondaryStats.SecondaryStat)[]).concat([mod.primaryStat])
    .reduce((acc, stat) => acc.plus(stat.bigValue.mul(statScore[stat.type])) , Big(setScore[mod.set])).toFixed(2));
  })
)
  ...
  (mod: Mod) => {
    ...
    if (['arrow', 'triangle', 'cross'].includes(mod.slot) &&
        (['Defense %',
          ...
          'Potency %',
          'Tenacity %',
          'Speed'] as PrimaryStats.GIMOStatNames[]).includes(mod.primaryStat.type))
      return 0;
      const extraBadRolls = mod.secondaryStats.reduce((acc, stat) => {
      ...
      return mod.secondaryStats.reduce((acc, stat) => acc.plus(stat.score.value.mul(stat.rolls).mul(isOffenseSecondary(stat.type) ? 1 : 0)), Big(0)).div(offenseRolls+extraBadRolls).toNumber();
  })
)
...
modScores.push(new ModScore(
  ...
  (mod: Mod) => {
    ...
    if (!['Defense %', 'Health %'].includes(mod.set))
      return 0;
    if (['arrow', 'triangle', 'cross'].includes(mod.slot) &&
        (['Critical Chance %',
          'Critical Damage %',
      ...
      return mod.secondaryStats.reduce((acc, stat) => acc.plus(
        stat.score.value.mul(stat.rolls).mul(isDefenseSecondary(stat.type) ? 1 : 0)
        ), Big(0))
        .div(defenseRolls+extraBadRolls).toNumber();
  })
)